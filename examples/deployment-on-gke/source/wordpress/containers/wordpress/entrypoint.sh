...
# generates random keys and salts if they are missing
# allow any of environment variables (ie, "AUTH_KEY")
# But not with a "WORDPRESS_" prefix (ie, "WORDPRESS_AUTH_KEY")
UNIQUES=(
  AUTH_KEY
  SECURE_AUTH_KEY
  LOGGED_IN_KEY
...
for unique in "${UNIQUES[@]}"; do
  ...
  if [ -z "$unique_value" ]; then
    ...
    export ${unique}=$(head -c1M /dev/urandom | sha1sum | cut -d' ' -f1)
    echo "auto-generated environment variable ${unique}"
  fi
done
